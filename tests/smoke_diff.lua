...
h.assert_eq(
  same,
  "(no changes)",
  "identical content is reported plainly instead of an empty string"
)

local root = diff.unified(nil, "brand new content")
h.assert_true(
  root:find("brand new content") ~= nil,
  "a nil old_content (root commit) diffs against empty, showing the whole file as additions"
)
